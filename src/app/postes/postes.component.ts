...
interface StatisticsResponse {
  [period: string]: { [category: string]: number }; // Adjust the type if needed
}
...
export class PostesComponent  implements OnInit{
 selectedDate: string = '';
  ...
  chart?: Chart<'line', number[], string>; // Chart instance
  chartData?: ChartData<'line', number[], string>; // Chart data
  ...
  public chartPoste: ApexOptions = {
    ...
    xaxis: {
      labels: {
        ...
        rotate: -45, // Rotation des labels pour mieux les afficher
        style: {
          colors: '#000',  // Couleur des labels
          fontSize: '10px',  // Taille de la police des labels
          fontFamily: 'Helvetica, Arial, sans-serif', // Police des labels
      ...
      axisBorder: {
        show: true,
        color: '#d1d1d1', // Bordure de l'axe X
      },
      axisTicks: {
        show: true,
        color: '#d1d1d1', // Couleur des ticks de l'axe X
      },
    },
    grid: {
      show: true,
      borderColor: '#f1f1f1',  // Couleur de la grille
      strokeDashArray: 0, // Ligne continue pour la grille
    },
    plotOptions: {
      area: {
        fillTo: 'origin', // Remplissage sous la courbe
      }
    },
  };
...
const selectedDate = new Date(new Date(selectedDateStr).toUTCString());
  ...
  fetchStatistics(category: string, period: string, date: string,): void {
    ...
    this.statisticsService.getPostesCatgeStatistics(category,period, formattedDate ).subscribe
          ...
          options: {
            ...
            scales: {
              ...
              y: {
                ...
                ticks: {
                  suggestedMin: 0 // Utiliser suggestedMin pour éviter que l'axe commence à des valeurs négatives
                }
              }
            }
          } as ChartOptions<'line'> // Type assertion pour assurer le type correct de ChartOptions
        });
      } else {
        console.error('Canvas element with id "myChart" not found or chartData is undefined.');
      }
    }*/



 /*   updateChart(transformedData: { series: number[], labels: string[] }): void {
      this.chartPoste.series = transformedData.series;
      this.chartPoste.labels = transformedData.labels;
    }
  
    createChart(): void {
      const chartElement = document.getElementById('chartPoste');
      if (chartElement) {
        const chart = new ApexCharts(chartElement, this.chartPoste);
        chart.render();
      } else {
